{
  *** modified comparison function type to Integer of Object
  ...
  Generics version by Jani Matyas (jzombi)
...
Type TColor = (clRed, clBlack);

  
Type generic TRBTree<T> = class
  Type
    TCompareFunc = function(const Item1, Item2: T): Integer of Object;
    TRBNodeP = ^TRBNode;
    TRBNode = record
      k: T;
  ...
  Var
    private
      ...
      class procedure fast_erase(x: TRBNodeP);
    ...
    public
      ...
      function Find(const key: T): TRBNodeP;
      function Add(key: T): TRBNodeP;
      procedure Delete(z: TRBNodeP);
      
      property First: TRBNodeP read leftmost;
      property Last: TRBNodeP read rightmost;
      class procedure RBInc(var x: TRBNodeP);
      class procedure RBDec(var x: TRBNodeP);
end; { class TRBTree }
...
constructor TRBTree.Create(Compare: TCompareFunc);
begin
  inherited Create;
  compareFunc := Compare;
  root := nil;
...
class procedure TRBTree.fast_erase(x: TRBNodeP);
begin
...
function TRBTree.Find(const key: T): TRBNodeP;
var
  cmp: integer;
begin
  Result := root;
...
function TRBTree.Add(key: T): TRBNodeP;
var
  x, y, z, zpp: TRBNodeP;
  cmp: Integer;
begin
  ...
  while (x <> nil) do begin
    ...
    if (cmp < 0) then begin
      ...
      Result := x;
      Dispose(z); //a jzombi: memory leak: if we don't put it in the tree, we shouldn't hold it in the memory
      exit;
    end;
  end;
  z^.parent := y;
...
{ Pre: x <> last }
class procedure TRBTree.RBInc(var x: TRBNodeP);
var
  y: TRBNodeP;
begin
...
{ Pre: x <> first }
class procedure TRBTree.RBDec(var x: TRBNodeP);
var
  y: TRBNodeP;
begin
